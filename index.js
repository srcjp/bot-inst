...
async function login(ig) {
    ...
    try {
        ...
        console.warn('Não foi possível usar a sessão salva. Fazendo login completo.', e.message);
    }
...
async function postarNoInstagram() {
    ...
    try {
        ...
        const arquivosDeImagem = arquivosNaPasta.filter(file => /\.(png|jpg|jpeg)$/i.test(file));

        if (arquivosDeImagem.length === 0) {
            console.error(`Nenhuma imagem encontrada na pasta '${nomePastaDia}'. Pulando postagem.`);
            return;
        }
        
        console.log(`Foram encontradas ${arquivosDeImagem.length} imagens.`);
        const legenda = await fs.readFile(path.join(caminhoBase, 'texto.txt'), 'utf-8');
        
        const locations = await ig.location.search({ query: 'Londrina' });
        const localizacaoLondrina = locations.find(loc => loc.name.toLowerCase().includes('londrina') && loc.name.toLowerCase().includes('pr')) || locations[0];
        ...
        if (arquivosDeImagem.length === 1) {
            ...
            await ig.publish.photo({ file: imagemBuffer, caption: legenda, location: localizacaoLondrina });
        } else {
            console.log('Publicando álbum/carrossel...');
            const items = await Promise.all(
                arquivosDeImagem.map(async (img) => ({ file: await fs.readFile(path.join(caminhoBase, img)) }))
            );
            await ig.publish.album({ items, caption: legenda, location: localizacaoLondrina });
        }
        ...
        if (error.name === 'IgCheckpointError') {
            ...
            console.error('Acesse sua conta pelo celular/navegador para confirmar sua identidade.');
            console.error('******************************************************************');
        }
    }
}
...
cron.schedule('* * * * *', () => {
    ...
    const horaSP = parseInt(agora.toLocaleTimeString('pt-BR', { hour: '2-digit', timeZone: 'America/Sao_Paulo' }));
    const minutoSP = parseInt(agora.toLocaleTimeString('pt-BR', { minute: '2-digit', timeZone: 'America/Sao_Paulo' }));
    ...
    if (horaSP === 6 && minutoSP === 46 && !postDeHojeFeito) {
        console.log('Hora correta! (6:30). Iniciando postagem...');
        postarNoInstagram();
    }
});

console.log('Script iniciado. O agendador está rodando e vai verificar a hora a cada minuto.');
